...
folder-reverse|Die Namen der Verzeichnisse, in dem sich eine Datei befindet, kann �ber `folder1`, `folder2` usw. ausgegeben werden. (siehe `output`). Über diese Einstellung wird die Ausgabe umgedreht, d.h. in `folder1` wird der direkte Verzeichnisname statt dem Laufwerksbuchstaben (windows) oder dem Mountpiont ausgegeben|true oder false
